    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('finalize')
                ...
                ->visible(fn (Order $record) => $record->status === OrderStatus::Authorized)
                ->requiresConfirmation()
                ->modalDescription(fn (Order $record) => $record->allItemsWeighed()
                    ? 'Weights will be locked and the card charged for the actual weight. This cannot be undone.'
                    : 'Some items are not weighed yet.')
