 ...
 * the real cuts, prices and photos go in via /admin/categories, /admin/cut-options etc. in staging.
 */
class CategorySeeder extends Seeder
{
    public function run(): void
    {
        ...
        $this->cutOption($chicken, 'Whole (uncut)', CutStyle::Whole, 0, 0);
        $this->cutOption($chicken, 'Cut into 8 pieces', CutStyle::CurryCut, 100, 0);
        $this->cutOption($chicken, 'Boneless breast', CutStyle::Boneless, 200, 1);
        $this->offalOption($chicken, 'Keep giblets', 0);
        $this->offalOption($chicken, 'Discard giblets', 0);

        $this->cutOption($goat, 'Curry cut', CutStyle::CurryCut, 0, 0);
        $this->cutOption($goat, 'Chops', CutStyle::Chops, 300, 1);
        $this->cutOption($goat, 'Boneless', CutStyle::Boneless, 400, 2);
        ...
        $this->cutOption($beef, 'Curry cut', CutStyle::CurryCut, 0, 0);
        $this->cutOption($beef, 'Steaks', CutStyle::Steak, 300, 1);
        $this->cutOption($beef, 'Mince', CutStyle::Mince, 150, 0);
    ...
    private function cutOption(Category $category, string $name, CutStyle $style, int $extraPriceCents, int $extraLeadTimeDays): void
    {
        $option = CutOption::query()->firstOrNew(['category_id' => $category->id, 'name' => $name]);
        $option->fill(['cut_style' => $style, 'extra_price_cents' => $extraPriceCents, 'extra_lead_time_days' => $extraLeadTimeDays, 'is_active' => true]);
        $option->save();
    }
